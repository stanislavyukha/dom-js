document
  .querySelectorAll(".calculator .digits")
...
document
  .querySelectorAll(".calculator .operators")
...
function operatorPressed(ev) {
  ...
  if (
    btn.innerText != display.value.charAt(display.value.length - 1) &&
    Number.isNaN(lastChar) === true
  ) {
    display.value = display.value.replace(/.$/, btn.innerText);
  } else if (btn.innerText === "+/-" && display.value.charAt(0) == "-")
    display.value = display.value.replace(display.value.charAt(0), "");
  else if (btn.innerText === "+/-" && display.value.charAt(0) != "-")
    display.value = display.value.replace(display.value.charAt(0), "-");
  ...
  else if (btn.innerText === "1/x" && display.value != "0")
    display.value = 1 / display.value;
  else if (btn.innerText === "%") display.value = display.value + "/100";
  else if (btn.innerText === display.value.charAt(display.value.length - 1))
    display.value += "";
...
document
  .querySelector(".calculator .adjust")
  .addEventListener("click", adjustPressed);
